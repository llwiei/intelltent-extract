...
class ContentExtractor:
    ...
    def extract(self, selector, host='', body_xpath='', with_body_html=False, use_visiable_info=False):
        ...
        for node in iter_node(body):
            ...
            images_list = node.getparent().xpath('.//img/@src')
            ...
            if host:
                ...
                for url in images_list:
                    if not url.endswith("svg"):
                        new_images_list.add(pad_host_for_images(host, url))

                images_list = list(new_images_list)
            else:
                for url in images_list:
                    if not url.endswith("svg"):
                        new_images_list.add(url)
                images_list = list(new_images_list)
            ...
            if element_flag in self.element_text_cache: # 直接读取缓存的数据，而不是再重复提取一次
                ...
                for text in element.xpath('.//text()'):
                    ...
                    if not text:
                        continue
                    # if text.tag == "a":
                    #     newtext = "START" + text + "END"#后续处理
                    #     clear_text = re.sub(' +', ' ', newtext, flags=re.S)
                    #     print(clear_text)
    ...
    def calc_text_density(self, element):
        ...
        #ti_text = self.compare_text(self.get_all_text_of_element(element))
    ...
    def calc_new_score(self):
        ...
        for node_hash, node_info in self.node_info.items():
            ...
            #     node_info['sbdi'])
            score = node_info['density'] * (node_info['text_tag_count'] + 2) * np.log(
                    node_info['sbdi'])
